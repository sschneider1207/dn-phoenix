...
namespace Phoenix
{
    ...
    public class Socket
    {
        ...
        private IList<Action<string, short>> _closeCallbacks;
        ...
        {
            _openEventHandler = new EventHandler(OnConnOpen);
            _closedEventHandler = new EventHandler(OnConnClose);
            _errorEventHandler = new EventHandler<ErrorEventArgs>(OnConnError);
            _messageEventHandler = new EventHandler<MessageReceivedEventArgs>(OnConnMessage);

            _openCallbacks = new List<Action>();
            _closeCallbacks = new List<Action<string, short>>();
        ...
        public void Disconnect(Action callback = null, ushort? code = null, string reason = null)
        {
            if (_conn != null)
            {
                _conn.Closed -= _closedEventHandler;

                if (_conn.State != WebSocketState.Closed)
                {
                    if (code != null)
                    {
        ...
        public void Connect()
        {
            ...
            _conn = new WebSocket(EndpointUrl()); //jfis - websocket-sharp specific
                                                  //
            _conn.Opened += _openEventHandler;
            _conn.Error += _errorEventHandler;
            _conn.MessageReceived += _messageEventHandler;
            _conn.Closed += _closedEventHandler;
        ...
        public void OnClose(Action<string, short> callback)
        {
        ...
        //}
        internal EventHandler _openEventHandler;
        ...
        //}
        internal EventHandler _closedEventHandler;
        internal void OnConnClose(object sender, EventArgs e)
        {
            Log("transport", "close");

            TriggerChanError(); //jfis - trigger error event on all channels
            _heartbeatTimer.Stop(); //jfis - no conn so stop heartbeat
            _reconnectTimer.SetTimeout(); //jfis - no conn so start trying to reconnect

            if (e is ClosedEventArgs)
            {
                var c = (ClosedEventArgs)e;
                foreach (var cb in _closeCallbacks)
                {
                    cb(c.Reason, c.Code);
                }
            }
        ...
        //}
        internal EventHandler<ErrorEventArgs> _errorEventHandler;
        internal void OnConnError(object sender, ErrorEventArgs e)
        {
            Log("transport", "error");

            TriggerChanError(); //jfis - trigger error event on all channels
            foreach (var cb in _errorCallbacks)
            {
                cb(e.Exception.Message, e.Exception);
            }
        }
        ...
        internal void TriggerChanError()
        {
            ...
            {
                c.Trigger(Phoenix.CHANNEL_EVENT_ERROR);
            }
        }
        ...
        internal string ConnectionState()
        {
            ...
            switch (_conn.State) //jfis - websocket-sharp specific
            {
        ...
        //}
        //internal void OnConnMessage(JsonWebSocket sender, Message e)
        //{
        //    Log("receive", $"{e.Payload["status"] ?? ""} {e.Topic} {e.Event} ({ e.Ref })", e.Payload);
        //    //jfis - pass on to appropriate channels
        //    _channels.Where(c => c.IsMember(e.Topic))
        //        .ToList()
        //        .ForEach(c => c.Trigger(e.Event, e.Payload, e.Ref));
        //    foreach (var cb in _messageCallbacks)
        //    {
        //        cb(e);
        //    }
        //}
        internal EventHandler<MessageReceivedEventArgs> _messageEventHandler;
        internal void OnConnMessage(object sender, MessageReceivedEventArgs e)
        {
            Log("DATA", e.Message);
